...
const priorityConfig: Record<string, { label: string; emoji: string; bg: string; text: string }> = {
  hot: { label: 'Hot', emoji: 'ðŸ”¥', bg: 'bg-red-100', text: 'text-red-700' },
  warm: { label: 'Warm', emoji: 'ðŸŸ¡', bg: 'bg-amber-100', text: 'text-amber-700' },
  cold: { label: 'Cold', emoji: 'ðŸ”µ', bg: 'bg-blue-100', text: 'text-blue-700' },
}
...
export default function LeadsKanban() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-8rem)] max-w-full flex-col overflow-hidden">
      {/* Header */}
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Leads Pipeline</h1>
          <p className="text-slate-500 mt-1">
            {leads.length} lead{leads.length !== 1 ? 's' : ''} â€¢ Drag to update status
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={fetchLeads}
            disabled={loading}
            className="flex items-center gap-2 rounded-lg px-3 py-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 disabled:opacity-50"
            title="Refresh leads"
      ...
      <div className="-mx-4 flex-1 overflow-x-auto px-4 sm:mx-0 sm:px-0">
        <div className="flex min-h-[32rem] w-max min-w-full gap-4 pb-4">
          {stages.map((stage) => (
            <div 
              key={stage.id}
              className="flex w-[18rem] shrink-0 flex-col rounded-xl bg-slate-100 sm:w-80"
                    ...
                    <div className="space-y-2 text-sm text-slate-500">
                      ...
                      {lead.notes && (
                        <p className="text-xs text-slate-400 truncate mt-2">
                          ðŸ“ {lead.notes.substring(0, 40)}{lead.notes.length > 40 ? '...' : ''}
                        </p>
                      )}
                    </div>
                  </div>
                      ...
                      <div key={activity.id} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">
                            {formatDateTime(activity.created_at)} â€¢ {activity.created_by}
                          </p>
                        </div>
                      </div>
                    ))}
