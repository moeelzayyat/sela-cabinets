...
export async function listUsers(): Promise<AdminUser[]> {
  ...
  const result = await pool.query(
    `SELECT id, email, full_name, phone, password_hash, provider, is_admin, is_active, created_at
...
export async function findUserByEmail(email: string): Promise<AdminUser | null> {
  await ensureAdminUsersTable()
  const result = await pool.query(
    'SELECT id, email, full_name, phone, password_hash, provider, is_admin, is_active, created_at FROM admin_users WHERE email = $1 LIMIT 1',
...
export async function findUserById(id: number): Promise<AdminUser | null> {
  await ensureAdminUsersTable()
  const result = await pool.query(
    'SELECT id, email, full_name, phone, password_hash, provider, is_admin, is_active, created_at FROM admin_users WHERE id = $1 LIMIT 1',
    [id]
  )
  return result.rows[0] || null
}
...
export async function createUser(email: string, password: string, fullName?: string, phone?: string, isAdmin = false) {
  ...
  const result = await pool.query(
    `INSERT INTO admin_users (email, full_name, phone, password_hash, provider, is_admin)
     ...
     RETURNING id, email, full_name, phone, password_hash, provider, is_admin, is_active, created_at`,
    [email.toLowerCase().trim(), fullName || null, phone || null, passwordHash, isAdmin]
  )
  return result.rows[0] || null
}

export async function upsertGoogleUser(email: string, fullName?: string) {
  await ensureAdminUsersTable()
  const result = await pool.query(
    `INSERT INTO admin_users (email, full_name, provider, is_admin)
     ...
     ON CONFLICT (email) DO UPDATE SET
       ...
       is_admin = admin_users.is_admin
     RETURNING id, email, full_name, phone, password_hash, provider, is_admin, is_active, created_at`,
    [email.toLowerCase().trim(), fullName || null]
  )
  return result.rows[0] || null
}
